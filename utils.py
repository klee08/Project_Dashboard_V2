...
def download_px_history_to_csv(symbol, path_to_save):
    sql_query = f"""
    SELECT distinct date as timestamp, open, high, low, close, close as adjusted_close, volume, 0 as dividend_amount, 1 as split_coefficient
    FROM PX_HISTORY where symbol = '{symbol}' order by date desc
    """
    price_data = pd.read_sql_query(sql_query, eft_data_connection_string)
    price_data = price_data.set_index('timestamp')
    price_data.to_csv(path_to_save)

def download_save(symbol, path_to_save, logger=None):
    if logger:
        logger.append_log("Starting download ", symbol)
    else:
        print("Starting download ", symbol)
    download_px_history_to_csv(symbol, path_to_save)
    if logger:
...
def get_wr_mod(high, low, close, lookback):
...
def get_williamR(df, col_name, intervals):
    ...
    for i in tqdm(intervals):
        ...
        #df["wr_" + str(i)] = ((df[i, 'high'] - df[i, 'close']) / df[i, 'high'] - df[i, 'low']) * -100
        
        #df["wr_" + str(i)] = WilliamsRIndicator(df['high'], df['low'], df['close'], i, fillna=True)
        df["wr_" + str(i)] = get_wr_mod(df['high'], df['low'], df['close'], i)
...
def get_DMI(df, col_name, intervals):
    ...
    df_ss = sdf.retype(df)
    for i in tqdm(intervals):
        df['dmi_'+str(i)] = adx(df['high'], df['low'], df['close'], window=i, fillna=True)
